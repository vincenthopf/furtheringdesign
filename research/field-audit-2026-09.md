...
## What current benchmarks measure and miss

- Generative UI Bench evaluates 46 briefs across 70 canonical components and separates parse success, render success, reference correctness, and rubric completeness: https://github.com/thesysdev/generative-ui-bench
- DesignBench's failure annotation guide distinguishes compile failure, layout disorder, wrong position, colour, text, size, missing elements, wrong edit target, partial edits, and unnecessary modifications: https://github.com/WebPAI/DesignBench/blob/main/assets/Failure_Annotation_Guideline.md

These benchmarks are useful because they decompose failure instead of reporting one screenshot score. They still leave a product-level gap: a rendered and rubric-complete interface can be generic, strategically wrong, inaccessible outside the captured state, destructive to a working repository, or supported by weak subjective evidence. The protocol retains benchmark-compatible render and structure signals but places them inside intent, change-integrity, state-coverage, uncertainty, and human-review contracts.
