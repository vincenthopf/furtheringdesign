...
and no unresolved hard obligation
...
## Phase 6: repair without drift

Selection does not end the loop. The runtime can convert weak, failed, missing, or uncertain evidence into a bounded revision plan. The plan orders findings by obligation class, severity, intent weight, quality gap, and confidence. It carries forward the immutable baseline and path contract and names the exact states and dimensions that must be captured again.

A revision remains a bounded patch while it preserves the candidate thesis. When evidence shows structural intent misalignment, the plan returns `fork-candidate` so the original decision record remains intact. This prevents a sequence of local fixes from producing an undocumented fourth direction.

Unknown evidence never contributes a positive score. It remains unresolved until a deterministic check, behavioural observation, calibrated judgment, or production outcome resolves it.
