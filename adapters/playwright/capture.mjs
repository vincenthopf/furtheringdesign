...
function signals(candidateId, state, browser, axe, outline, focus, url) {
  ...
  const webkitTabLimitation = browser === "webkit" && !focus.length && outline.interactiveTargetCount > 0;
  return [
    ...
    signal(candidateId, state.id, browser, "focus", "A", "accessibility", webkitTabLimitation ? "unknown" : !focus.length || invisibleFocus.length ? "fail" : "pass", webkitTabLimitation ? 0.5 : !focus.length || invisibleFocus.length ? 0.4 : 0.95, webkitTabLimitation ? 0.99 : 0.9, webkitTabLimitation ? "minor" : !focus.length || invisibleFocus.length ? "major" : "note", invisibleFocus[0]?.ref || focus[0]?.ref || "document", webkitTabLimitation ? `${outline.interactiveTargetCount} interactive targets are present, but Playwright WebKit did not advance focus with Tab.` : `${focus.length} focus stops; ${invisibleFocus.length} invisible.`, webkitTabLimitation ? "Playwright WebKit on macOS follows Safari's full-keyboard-access limitation, so this automated Tab trace is unresolved rather than evidence that controls are absent." : "The task needs a visible keyboard path.", webkitTabLimitation ? "Use Chromium and Firefox traces plus manual Safari full-keyboard-access review." : !focus.length || invisibleFocus.length ? "Repair focusability, order, and visibility." : "Run task-specific keyboard review."),
